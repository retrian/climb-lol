...
// Error boundary wrapper for DB calls
async function safeDb<T>(query: any, fallback: T): Promise<T> {
...
function displayRiotId(p: Partial<Player>) {
  const gn = (p.game_name ?? '').trim()
  const tl = (p.tag_line ?? '').trim()
  if (gn && tl) return `${gn}#${tl}`
  return p.puuid || 'Unknown'
}

function getOpggUrl(player: Partial<Player>) {
...
function TeamHeaderCard({ name, description, visibility, lastUpdated, cutoffs, bannerUrl, actionHref, actionLabel, secondaryActionHref, secondaryActionLabel }: any) {
    return (
...
export default async function LeaderboardDetail({
  ...
  // Fetch Top 50 players, cutoffs, and latest games (RPC)
  // 5. Error Boundary: getChampionMap wrapped in catch to prevent page crash
  const [
    champMap,
    playersRaw,
    cutsRaw,
  ...
  // 4. Optimization: Use Set for filtering
  const missingPuuids = Array.from(gamePuuids).filter(p => !top50Set.has(p))
  const allRelevantPuuids = Array.from(new Set([...top50Puuids, ...missingPuuids]))
  ...
  const [
    ...
    // 2. Missing IDs: Fetch 'id' so we can construct a valid Player object
    missingPuuids.length > 0 ? safeDb(supabase.from('players').select('id, puuid, game_name, tag_line').in('puuid', missingPuuids), [] as PlayerBasicRaw[]) : [],
  ...
  // 1. Merge Players (Type Safety Fix)
  // We use the ID from the database for missing players to fully satisfy the Player interface
  ...
  const playersSorted = [...players].sort((a, b) => {
    // 3. Sorting Fix: Convert null (from Map) to undefined (for compareRanks)
    const rankA = rankBy.get(a.puuid) ?? undefined
    const rankB = rankBy.get(b.puuid) ?? undefined
