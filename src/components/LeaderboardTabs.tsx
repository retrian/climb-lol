        ...
        {tabs.map((tab) => {
          ...
          if (tab.id === activeTab) {
            return (
              <span
                key={tab.id}
                aria-current="page"
                className="inline-flex items-center rounded-full bg-slate-900 px-3.5 py-1.5 text-white shadow-sm transition-colors duration-200 cursor-default dark:bg-white dark:text-slate-900"
              >
                {tab.label}
              </span>
            )
          ...
          return (
            <Link
              ...
              href={href}
              scroll={false}
              className="inline-flex items-center rounded-full px-3.5 py-1.5 transition-colors duration-200 hover:bg-slate-100 hover:text-slate-900 dark:hover:bg-slate-800 dark:hover:text-white"
            >
              {tab.label}
            </Link>
          )
